...
const Choices: React.FC<{ choices: Choice[]; onChoiceSelected: (c: Choice) => void }> = ({ choices, onChoiceSelected }) => {
  ...
  const buttons = choices.map((choice) => {
    return (
      <button
        ...
        onClick={(e) => {
          choiceListRef.current?.scrollTo({ left: 0, behavior: "smooth" });
          e.preventDefault();
  ...
  return (
    <div ref={choiceListRef} className="flex inline-flex shrink-0 flex-row space-x-4 overflow-x-auto rounded-md p-4" role="group">
      {buttons}
    </div>
  );
};
